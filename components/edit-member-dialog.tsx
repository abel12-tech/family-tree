...
export default function EditMemberDialog({ children, member, onClose }: EditMemberDialogProps) {
  const [open, setOpen] = useState(false);
  const { updateFamilyMember } = useFamilyTreeStore();
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      const response = await fetch(`http://localhost:3001/familyMembers/${member.id}`, {
        method: 'PATCH',
        headers: {
